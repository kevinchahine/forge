		...
		namespace movegen
		{
			void runChessEngine(bp::child& chessEngine, bp::opstream& out, bp::ipstream& in)
			{
				...
				if (!fs::exists(chessEnginePath)) {
					...
					//cout << termcolor::push << termcolor::green
					//	<< "uci" << termcolor::pop << endl;

					out << "uci" << endl;

					// Ignore engine output until "uciok"
					cout << termcolor::push << termcolor::yellow;
				...
				const forge::Position & pos)
			{
				...
				if (!chessEngine.running()) {
					cout << termcolor::push << termcolor::bright_green
				...
				const std::vector<Move>& moves)
			{
				...
				///cout << termcolor::bright_magenta << "posSet.size() " << posSet.size() << endl;
				...
				std::set<forge::MovePositionPair>& faults)
			{
				...
				cout << endl << termcolor::push << termcolor::green
				...
				cout << endl << termcolor::push << termcolor::yellow
				...
				cout << endl << termcolor::push << termcolor::red
			...
			void stockfishAndForge()
			{
				...
				while (frontier.size()) {
					...
					cout << termcolor::magenta << sfMoves.size() << " moves generated by stockfish\n";
					movePos.position.board().printMini();
					...
					// --- Skip Finished Games ---
					if (sfMoves.empty()) {
						cout << termcolor::yellow << "Terminal node\n";
						movePos.position.board().printMini();
